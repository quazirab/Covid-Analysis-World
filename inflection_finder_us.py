...
def inflection_finder(province_state,county):
...
def store_all():
    ...
    for province_state in province_states:
        ...
        for county in counties:
            if county == None:
                county = 'Null'
            inflection_result,inflection_date,slope_max = inflection_finder(province_state,county)
            inflections.append([province_state,county,inflection_result,inflection_date,slope_max])

    inflection_df = pd.DataFrame(inflections,columns=['province_state','county','inflection_result','inflection_date','slope_max'])
